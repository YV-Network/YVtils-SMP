...
public class Logger {
    ...
    public void writer(String log, String event) throws IOException {
        ...
        File dataFolder1 = new File( SMPPlugin.getInstance().getDataFolder() + "/Logs/");
        if(!dataFolder1.exists())
        {
            dataFolder1.mkdir();
        }

        File dataFolder2 = new File( SMPPlugin.getInstance().getDataFolder() + "/Logs/" + event);
        ...
        File saveTo = new File(SMPPlugin.getInstance().getDataFolder() + "/Logs/" + event, sdf2.format(timestamp) + ".EventLog.log");
        if(!saveTo.exists()) {
            try {
